...
Fence resolves capture policy in this order:

1. built-in safe defaults;
2. optional user configuration;
3. optional project restrictions;
4. explicit `fence run` flags.

On Unix the user file is
`$XDG_CONFIG_HOME/fence/config.toml`, falling back to
`$HOME/.config/fence/config.toml`. On Windows it is
`%APPDATA%/Fence/config.toml`. `FENCE_CONFIG_FILE` selects an explicit user
file.

The project file is `.fence/config.toml` at the worktree root. A project file
