...
use anchor_windows::{DirectoryHandle, MutationRoot, SymbolicLinkData};
    ...
    let items = writes
        ...
        .map(|(index, write)| {
            Ok(JournalItem {
                ...
                stage_name: format!(".anchor-stage-{id}-{index}"),
                backup_name: format!(".anchor-backup-{id}-{index}"),
...
fn verify_index_node(
    ...
    match expected {
        ...
        IndexCapture::Present {
            ...
            if node.metadata().kind != anchor_windows::NodeKind::RegularFile {
                return Ok(false);
            }
...
impl Presence {
    ...
    fn from_entry(entry: Option<&ManifestEntry>) -> Result<Self, RestoreError> {
        ...
        let manifest = Manifest::new(
            anchor_core::PathEncoding::WindowsWtf16Le,
            vec![entry.clone()],
            Coverage {
