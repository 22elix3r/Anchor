...
#[cfg(windows)]
use anchor_windows::{DirectoryEntry, DirectoryHandle, NodeHandle, NodeKind, RootHandle};
#[cfg(not(windows))]
...
pub struct FrozenGitPolicy {
    ...
    pub root_anchorignore: FrozenIgnoreFile,
...
impl FrozenGitPolicy {
    ...
    pub fn validate(&self) -> Result<(), GitError> {
        ...
        validate_ignore_file(self.root_anchorignore)?;
    ...
    pub fn compile(
        ...
        if let FrozenIgnoreFile::Regular(blob) = self.root_anchorignore {
            let bytes = store.get(blob.object, blob.raw_size)?;
            anchor_search.add_patterns_buffer(
                &bytes,
                worktree_root.join(".anchorignore"),
                Some(worktree_root),
                parser,
            );
        }
    ...
    pub fn drift_from(&self, observed: &Self) -> PolicyDrift {
        PolicyDrift {
            ignore_sources_changed: self.global_ignore != observed.global_ignore
                ...
                || self.root_anchorignore != observed.root_anchorignore,
    ...
    pub fn referenced_objects(&self) -> BTreeSet<(ObjectId, u64)> {
        ...
        collect_file_object(self.root_anchorignore, &mut objects);
        objects.extend(
...
impl GitContext {
    /// Freeze every source that affects Anchor's inclusion policy.
    ///
    /// The scan is read-only and stores source bytes as normal immutable Anchor objects. Callers
    ...
    pub fn observe_frozen_policy(
        ...
        let (gitignore_sources, root_anchorignore, nested_repositories) =
...
> {
    ...
    let mut anchorignore = FrozenIgnoreFile::Absent;
    ...
    while let Some((directory_path, directory)) = pending.pop() {
        ...
        for name in &first_names {
            ...
            if directory_path.is_root() && name == OsStr::new(".anchorignore") {
                if kind != ObservedKind::Regular {
                    return Err(GitError::UnsupportedIgnoreSource(
                        worktree_root.join(".anchorignore"),
                    ));
                }
                anchorignore = FrozenIgnoreFile::Regular(capture_capability_policy_file(
                    &directory,
                    name,
                    &worktree_root.join(".anchorignore"),
                    store,
    ...
    Ok((sources, anchorignore, nested))
}
...
> {
    ...
    while let Some((directory_path, directory)) = pending.pop() {
        ...
        for entry in &first {
            ...
            if directory_path.is_root() && entry.name == OsStr::new(".anchorignore") {
                if kind != ObservedKind::Regular {
                    return Err(GitError::UnsupportedIgnoreSource(
                        worktree_root.join(".anchorignore"),
                    ));
                ...
                let node = directory.open_child(entry)?;
                anchorignore = FrozenIgnoreFile::Regular(capture_windows_policy_file(
                    &node,
                    &worktree_root.join(".anchorignore"),
                    store,
                )?);
