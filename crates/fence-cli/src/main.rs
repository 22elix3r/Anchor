...
use fence_git::{GitContext, PolicyDrift};
use fence_session::{
    ...
    match cli.command {
        Commands::Run {
            ...
            let resolution = ConfigLoader::load(context.worktree_root())
                .into_diagnostic()
                .wrap_err("cannot resolve Fence configuration")?;
            ...
            eprintln!("Fence session {}: {:?}", result.session_id, result.state);
        ...
        Commands::Review { session } => {
            ...
            let action = fence_tui::review(&model)
                .into_diagnostic()
                .wrap_err("terminal review failed")?;
            let fence_tui::ReviewAction::RestoreSelected { index } = action else {
                return Ok(0);
            };
            let change = diff
                .changes
        ...
        Commands::Delete { session, yes } => {
            ...
            println!("deleted {id} recoverably; use `fence undelete {id}` to restore it");
            Ok(0)
        }
        ...
        Commands::Purge { session, yes } => {
            ...
            println!(
                "permanently removed session record {id}; unreachable data is reclaimed by `fence gc`"
            );
            Ok(0)
        }
    }
            ...
            if format == OutputFormat::Json {
                ...
                eprintln!(
                    "no change made; apply this exact preview with \
                     `fence rollback {session} --yes --expect-current {current_manifest}`"
                );
            }
            Ok(3)
        }
...
fn review_model(
    ...
    diff: &ManifestDiff,
    objects: &ObjectStore,
) -> Result<fence_tui::ReviewModel> {
    let files = diff
        .changes
        .iter()
        .map(|change| {
            Ok(fence_tui::ReviewFile {
...
fn review_content(
    entry: Option<&fence_core::ManifestEntry>,
    objects: &ObjectStore,
) -> Result<fence_tui::ReviewContent> {
    const MAX_TUI_FILE_BYTES: u64 = 2 * 1024 * 1024;
    let Some(entry) = entry else {
        return Ok(fence_tui::ReviewContent::Absent);
    };
    match &entry.node {
        ManifestNode::Regular {
            ...
            if *raw_size > MAX_TUI_FILE_BYTES {
                return Ok(fence_tui::ReviewContent::Binary { size: *raw_size });
            }
            let bytes = objects
                .get(*object, *raw_size)
                .into_diagnostic()
            ...
            Ok(fence_tui::ReviewContent::Text(
...
mod tests {
    ...
    fn rollback_and_restore_all_accept_machine_readable_preview() {
        let rollback =
            Cli::try_parse_from(["fence", "rollback", "session", "--format", "json"]).unwrap();
        assert!(matches!(
            rollback.command,
            Commands::Rollback {
        ...
        let restore =
            Cli::try_parse_from(["fence", "restore", "session", "--all", "--format", "json"])
                .unwrap();
        assert!(matches!(
            restore.command,
            Commands::Restore {
    ...
    fn restore_requires_exactly_one_scope() {
        assert!(Cli::try_parse_from(["fence", "restore", "session"]).is_err());
        assert!(
            Cli::try_parse_from(["fence", "restore", "session", "--all", "--file", "path"])
                .is_err()
        );
    }
}
