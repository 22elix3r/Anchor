...
impl RestoreService {
    ...
    pub fn restore_index(
        ...
        let record = RestorePlanRecord::index(
            ...
            session.worktree_key.clone(),
            after.repository.clone(),
            fence_core::NativeString::from_host(context.index_path().as_os_str()),
            after.index.clone(),
...
fn apply_batch(
    ...
    let mut journal = BatchRestoreJournal {
        ...
        session_id,
        worktree_root: fence_core::NativeString::from_host(worktree.as_os_str()),
        ...
        items: writes
            ...
            .map(|(index, write)| BatchJournalItem {
                ...
                stage_name: format!(".fence-stage-{transaction_id}-{index}"),
                backup_name: format!(".fence-backup-{transaction_id}-{index}"),
...
fn finish_batch(
    ...
    for item in &journal.items {
        ...
        let backup = validate_journal_temp_name(&item.backup_name, ".fence-backup-")?;
        ...
        let stage = validate_journal_temp_name(&item.stage_name, ".fence-stage-")?;
...
fn apply_one(
    ...
    let mut journal = RestoreJournal {
        ...
        worktree_root: Some(fence_core::NativeString::from_host(worktree.as_os_str())),
...
fn apply_index(
    ...
    let mut journal = IndexRestoreJournal {
        ...
        backup_name: Some(backup_name_text),
        worktree_key: Some(store.worktree_key.clone()),
        index_path: Some(fence_core::NativeString::from_host(index_path.as_os_str())),
...
fn recover_batch_journal(
    ...
    for (index, item) in journal.items.iter().enumerate() {
        ...
        let expected_stage = format!(".fence-stage-{transaction_id}-{index}");
        let expected_backup = format!(".fence-backup-{transaction_id}-{index}");
...
fn recover_file_journal(
    ...
    if journal.stage_name != format!(".fence-stage-{transaction_id}")
        || journal.backup_name.as_deref() != Some(&format!(".fence-backup-{transaction_id}"))
    {
        return Err(RestoreError::UnsafeJournalName);
    }
    let stage = validate_journal_temp_name(&journal.stage_name, ".fence-stage-")?;
...
fn recover_index_journal(
    ...
    let name = recorded_index
        ...
        .ok_or(RestoreError::UnsafeIndexPath)?;
    let parent = Dir::open_ambient_dir(parent_path, ambient_authority())?;
    if journal.backup_name.as_deref() != Some(&format!(".fence-index-backup-{transaction_id}")) {
        return Err(RestoreError::UnsafeJournalName);
    }
    let backup = validate_journal_temp_name(
        journal
...
fn validated_journal_worktree(
    store: &SessionStore,
    session_id: SessionId,
    recorded: &fence_core::NativeString,
...
pub enum RestoreError {
    ...
    UnsafeRootPath,
    #[error(
        "restore path addresses protected Git, Fence-store, submodule, or nested-repository data: {0:?}"
    )]
    ...
    #[error(transparent)]
    Git(#[from] fence_git::GitError),
    #[error(transparent)]
    Capture(#[from] fence_core::CaptureError),
    #[error(transparent)]
    Plan(#[from] fence_core::RestorePlanError),
    #[error(transparent)]
    PlanRecord(#[from] crate::restore_plan::RestorePlanError),
    #[error(transparent)]
    Path(#[from] fence_core::PlatformMismatch),
    #[error(transparent)]
    UnsafePath(#[from] fence_core::PathError),
    #[error(transparent)]
...
mod tests {
        ...
        for point in [
            ...
            assert!(
                fs::read_dir(root.path()).unwrap().all(|entry| {
                    ...
                    !name.to_string_lossy().starts_with(".fence-")
                }),
    ...
    fn subprocess_restore_crash_helper() {
        let root = PathBuf::from(
            std::env::var_os("FENCE_CRASH_WORKTREE")
                .expect("crash helper requires FENCE_CRASH_WORKTREE"),
        );
        let session = std::env::var("FENCE_CRASH_SESSION")
            .expect("crash helper requires FENCE_CRASH_SESSION")
            .parse()
    ...
    fn subprocess_crash_recovery_matrix() {
        ...
        for boundary in BOUNDARIES {
            ...
            let mut child = Command::new(std::env::current_exe().unwrap())
                ...
                .env("FENCE_CRASH_BOUNDARY", boundary)
                .env("FENCE_CRASH_MARKER", &marker)
                .env("FENCE_CRASH_WORKTREE", root.path())
                .env("FENCE_CRASH_SESSION", session.to_string())
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .spawn()
    ...
    fn plan_bound_recovery_refuses_a_non_derived_temporary_name() {
        ...
        let mut journal: BatchRestoreJournal =
            ciborium::de::from_reader(fs::read(&journal_path).unwrap().as_slice()).unwrap();
        journal.items[0].stage_name = ".fence-stage-attacker".to_owned();
        save_batch_journal(&journal_path, &journal).unwrap();
    ...
    fn batch_recovery_refuses_a_duplicate_path_in_an_untrusted_journal() {
        ...
        let item = |suffix: usize| BatchJournalItem {
            ...
            stage_name: format!(".fence-stage-{transaction_id}-{suffix}"),
            backup_name: format!(".fence-backup-{transaction_id}-{suffix}"),
        ...
        save_batch_journal(
            ...
            &BatchRestoreJournal {
                ...
                plan_id: None,
                transaction_id: None,
                worktree_root: fence_core::NativeString::from_host(root.path().as_os_str()),
                worktree_key: store.worktree_key.clone(),
                state: BatchJournalState::Prepared,
    ...
    fn refuses_an_unbound_interrupted_file_transaction() {
        ...
        let transaction_path = store
            ...
            .join(transaction_id.to_string());
        private_transaction_dir(&transaction_path).unwrap();
        let stage_text = format!(".fence-stage-{transaction_id}");
        let backup_text = format!(".fence-backup-{transaction_id}");
    ...
    fn refuses_an_unbound_batch_at_a_commit_point() {
        ...
        private_transaction_dir(&transaction_path).unwrap();
        let stage_text = format!(".fence-stage-{transaction_id}-0");
        let backup_text = format!(".fence-backup-{transaction_id}-0");
        let parent = Dir::open_ambient_dir(root.path(), ambient_authority()).unwrap();
        stage_node(&parent, OsStr::new(&stage_text), desired, store.objects()).unwrap();
        rename_noreplace(
            &parent,
    ...
    fn refuses_an_unbound_interrupted_index_transaction() {
        ...
        let transaction_path = store
            ...
            .join(format!("index-{transaction_id}"));
        private_transaction_dir(&transaction_path).unwrap();
        let backup_text = format!(".fence-index-backup-{transaction_id}");
        let journal_path = transaction_path.join("journal.cbor");
        save_index_journal(
            &journal_path,
            &IndexRestoreJournal {
    ...
    fn restore_refuses_a_regular_file_with_unmodeled_xattrs() {
        ...
        let length = rustix::fs::getxattr(&path, "user.fence-test", &mut value).unwrap();
...
mod windows_tests {
    ...
    use fence_core::NativeRelativePath;
