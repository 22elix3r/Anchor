...
use fence_core::{CaptureLimits, CaptureOptions};
...
impl ConfigLoader {
    ...
    ///
    /// The project file is `.fence/config.toml`. It can lower limits and disable risky
    ...
    pub fn load(worktree_root: &Path) -> Result<ConfigResolution, ConfigError> {
        ...
        let project_path = worktree_root.join(".fence").join("config.toml");
...
fn user_config_path() -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("FENCE_CONFIG_FILE") {
    ...
    #[cfg(windows)]
    {
        std::env::var_os("APPDATA")
            .map(PathBuf::from)
            .map(|root| root.join("Fence").join("config.toml"))
    }
    #[cfg(not(windows))]
    {
        std::env::var_os("XDG_CONFIG_HOME")
            ...
            .or_else(|| {
                std::env::var_os("HOME")
                    .map(PathBuf::from)
                    .map(|root| root.join(".config").join("fence").join("config.toml"))
            })
    }
}
