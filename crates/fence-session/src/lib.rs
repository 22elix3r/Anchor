...
pub use restore::{
    ...
    TransactionRecoveryReport, TransactionRecoveryService, WholeRestoreConflict, WholeRestoreMode,
    WholeRestoreResult,
};
...
impl SessionRunner {
    ...
    pub fn run(request: &RunRequest) -> Result<RunResult, SessionError> {
        ...
        #[cfg(windows)]
        let containment = fence_windows::KillOnCloseJob::new()?;
...
impl NativeStringWire {
    ...
    fn into_native(self) -> Result<NativeString, SessionError> {
        let encoding = match self.0 {
            1 => fence_core::PathEncoding::UnixBytes,
...
pub enum SessionError {
    ...
    EmptyCommand,
    #[error("another Fence session is active in this worktree: {0}")]
    ActiveSession(String),
    #[error("the shared Fence store is busy with another operation: {0}")]
    StoreBusy(String),
    ...
    ChildContainment {
        session_id: SessionId,
        source: fence_windows::WindowsError,
    },
    ...
    #[error(transparent)]
    Path(#[from] fence_core::PathError),
    #[error(transparent)]
    Platform(#[from] fence_core::PlatformMismatch),
    #[error(transparent)]
    Io(#[from] io::Error),
}
...
mod tests {
    ...
    fn current_capture_uses_retained_session_scope() {
        ...
        let drift = fence_core::ManifestDiff::between(&after, &current.manifest);
        assert_eq!(drift.changes.len(), 1);
        assert_eq!(drift.changes[0].kind, fence_core::ChangeKind::Modified);
    }
    ...
    fn external_global_ignore_bytes_are_frozen_for_every_endpoint() {
        ...
        let external = root
            ...
            .parent()
            .unwrap()
            .join(format!("fence-global-ignore-{}", SessionId::new()));
