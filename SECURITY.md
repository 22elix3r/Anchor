...
Anchor is pre-release software and has no supported stable version yet.
...
Anchor stores exact local file contents. Before sharing a reproduction, build a
minimal synthetic repository and remove the local store under `.git/anchor` from
archives.
